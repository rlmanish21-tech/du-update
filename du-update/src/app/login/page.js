    ...
    <div>
      <div style={{ textAlign:'center', marginBottom:'2rem' }}>
        ...
        <div style={{ fontSize:13, color:S.muted, marginTop:'.35rem' }}>
          New student?{' '}<Link href="/register" style={{ color:S.brand, fontWeight:700 }}>Create account</Link>
        </div>
      </div>

      {error && (
        <div style={{ background:'#FEE2E2', border:'1px solid #FECACA', borderRadius:8, padding:'11px 14px', fontSize:13, color:S.error, marginBottom:'1.25rem', display:'flex', alignItems:'center', gap:8 }}>
          <span>⚠️</span> {error}
        </div>
      )}
      ...
      <input style={{...I, marginBottom:'.5rem'}} type="password" placeholder="Enter your password"
      ...
      <button onClick={handleLogin} disabled={loading} style={{ width:'100%', padding:'13px', borderRadius:9, border:'none', background:S.brand, color:'#fff', fontSize:15, fontWeight:700, cursor:'pointer', opacity:loading?.7:1 }}>
...
export default function LoginPage() {
  return (
    <div style={{ minHeight:'85vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem', background:'#F7F9FF' }}>
      <div style={{ background:'#fff', border:'1px solid #DCE4F5', borderRadius:16, padding:'2.5rem 2rem', maxWidth:400, width:'100%', boxShadow:'0 4px 24px rgba(15,32,68,0.07)' }}>
