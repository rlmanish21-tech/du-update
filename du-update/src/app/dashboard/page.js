    ...
    <div style={{ maxWidth:940, margin:'0 auto', padding:'2.5rem 1.5rem' }}>
      ...
      <div style={{ background:S.navy, borderRadius:16, padding:'2rem', marginBottom:'2rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'1.25rem', flexWrap:'wrap' }}>
          <div style={{ width:60, height:60, borderRadius:'50%', background:S.brand, display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, fontWeight:900, color:'#fff', flexShrink:0 }}>
            {initial}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:20, fontWeight:800, color:'#fff' }}>{fullName}</div>
            <div style={{ display:'flex', flexWrap:'wrap', gap:'.5rem', marginTop:'.5rem' }}>
              {[['🏛️', collegeName], ['📚', courseName], ['📅', `Semester ${sem}`], ['📱', phone]].map(([icon, val]) => (
                <span key={val} style={{ background:'rgba(255,255,255,0.1)', color:'#C8D8F0', fontSize:12, padding:'4px 10px', borderRadius:20 }}>
                  {icon} {val}
                </span>
              ))}
            </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'.5rem', alignItems:'flex-end' }}>
            ...
            {isPremium
              ? <span style={{ fontSize:11, color:S.gold, fontWeight:700, background:'rgba(247,201,72,0.15)', padding:'4px 12px', borderRadius:20 }}>⭐ Premium</span>
              : <Link href="/#pricing" style={{ fontSize:12, color:'#fff', background:S.brand, padding:'6px 14px', borderRadius:7, textDecoration:'none', fontWeight:700 }}>Upgrade ₹499/yr</Link>
            }
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:'1rem', marginBottom:'1.5rem' }}>
        ...
        <Link href={`/courses/${courseId}/${sem}?tab=pyq`} style={{ textDecoration:'none' }}>
          <div style={{ background:S.brand, borderRadius:14, padding:'1.5rem', color:'#fff' }}>
            ...
            <div style={{ fontSize:12, fontWeight:700, marginTop:'.75rem', background:'rgba(255,255,255,0.2)', display:'inline-block', padding:'4px 12px', borderRadius:20 }}>Open PYQs →</div>
          </div>
        </Link>
        ...
        <div style={{ background:'#fff', border:`1.5px solid ${S.border}`, borderRadius:14, padding:'1.5rem', opacity:.65 }}>
          <div style={{ fontSize:28, marginBottom:'.5rem' }}>📅</div>
          ...
          <div style={{ fontSize:11, fontWeight:700, color:'#fff', marginTop:'.75rem', background:S.muted, display:'inline-block', padding:'4px 12px', borderRadius:20 }}>Coming Soon</div>
        </div>
        ...
        <div style={{ background:'#fff', border:`1.5px solid ${S.border}`, borderRadius:14, padding:'1.5rem', opacity:.65 }}>
          ...
          <div style={{ fontSize:12, color:S.muted, marginTop:'.3rem' }}>From Delhi University</div>
          <div style={{ fontSize:11, fontWeight:700, color:'#fff', marginTop:'.75rem', background:S.muted, display:'inline-block', padding:'4px 12px', borderRadius:20 }}>Coming Soon</div>
        </div>
      </div>
        ...
        <div style={{ display:'flex', gap:'.6rem', flexWrap:'wrap' }}>
          {[1,2,3,4,5,6].map(s => (
            <Link key={s} href={`/courses/${courseId}/${s}`} style={{ textDecoration:'none' }}>
              <div style={{ padding:'9px 18px', borderRadius:8, fontSize:14, border:`1.5px solid ${s===sem ? S.brand : S.border}`, background:s===sem ? '#FFF8F5' : S.off, color:s===sem ? S.brand : S.muted, fontWeight:s===sem ? 700 : 500 }}>
                Sem {s}{s===sem ? ' ✓' : ''}
              </div>
            </Link>
          ))}
