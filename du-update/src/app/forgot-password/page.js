...
export default function ForgotPasswordPage() {
  ...
  return (
    <div style={{ minHeight:'85vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem', background:S.off }}>
      <div style={{ background:'#fff', border:`1px solid ${S.border}`, borderRadius:16, padding:'2.5rem 2rem', maxWidth:400, width:'100%', boxShadow:'0 4px 24px rgba(15,32,68,0.07)', textAlign:'center' }}>
        ...
        {!sent ? (
          <>
            ...
            {error && (
              <div style={{ background:'#FEE2E2', borderRadius:8, padding:'10px 14px', fontSize:13, color:'#dc2626', marginBottom:'1rem', textAlign:'left' }}>{error}</div>
            )}
            ...
            <button onClick={handleReset} disabled={loading} style={{ width:'100%', padding:'12px', borderRadius:9, border:'none', background:S.brand, color:'#fff', fontSize:15, fontWeight:700, cursor:'pointer', opacity:loading?.7:1 }}>
              {loading ? 'Sending...' : 'Send Reset Link'}
            </button>

            <div style={{ marginTop:'1.25rem' }}>
              <Link href="/login" style={{ fontSize:13, color:S.muted, textDecoration:'none' }}>
                ← Back to Login
              </Link>
            </div>
          </>
          ...
          <>
            ...
            <p style={{ fontSize:14, color:S.muted, lineHeight:1.6, marginBottom:'1.5rem' }}>
              We've sent a password reset link to <strong style={{ color:S.navy }}>{email}</strong>.
              Click the link in that email to set a new password.
            </p>
            <p style={{ fontSize:12, color:S.muted, marginBottom:'1.5rem' }}>
              Didn't receive it? Check your spam folder or{' '}
              <button onClick={() => setSent(false)} style={{ background:'none', border:'none', color:S.brand, fontWeight:700, cursor:'pointer', fontSize:12 }}>
                try again
              </button>.
            </p>
